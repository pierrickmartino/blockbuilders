...
def read_users(session: SessionDep, skip: int = 0, limit: int = 100) -> Any:
    ...
    count_statement = select(func.count()).select_from(app_User)
    count = session.exec(count_statement).one()

    statement = select(app_User).offset(skip).limit(limit)
...
def read_user_by_id(user_id: uuid.UUID, session: SessionDep, current_user: CurrentUser) -> Any:
    ...
    """
    user = session.get(app_User, user_id)
...
def update_user(
    *,
    ...
    db_user = session.get(app_User, user_id)
    if not db_user:
        raise HTTPException(
            status_code=404,
